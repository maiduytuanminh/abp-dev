...
function TenantBox({
  ...
  const findTenant = () => {
    ...
    getTenant(tenantName).then(({ success, ...data }) => {
      if (!success) {
        Alert.alert(
          i18n.t('SmartSoftwareUi::Error'),
          i18n.t('SmartSoftwareUiMultiTenancy::GivenTenantIsNotAvailable', {
            0: tenantName,
          }),
          [{ text: i18n.t('SmartSoftwareUi::Ok') }]
        );
        return;
      }
      setTenant(data);
      ...
      >
        <View style={{ flex: 1 }}>
          <Text style={styles.title}>
            {i18n.t('SmartSoftwareUiMultiTenancy::Tenant')}
          </Text>
          <Text style={styles.tenant}>
            {tenant.name
              ? tenant.name
              : i18n.t('SmartSoftwareUiMultiTenancy::NotSelected')}
          </Text>
        </View>
        <Button
          style={{
        ...
        >
            {i18n.t('SmartSoftwareUiMultiTenancy::Switch')}
        </Button>
      </Box>
      {showTenantSelection ? (
        <Box
        ...
        >
          <FormControl my="2" width={350}>
            <FormControl.Label>
              {i18n.t('SmartSoftwareUiMultiTenancy::Name')}
            </FormControl.Label>
            <Input
              autoCapitalize="none"
              value={tenantName}
          ...
          <Text style={styles.hint}>
            {i18n.t('SmartSoftwareUiMultiTenancy::SwitchTenantHint')}
          </Text>
          <View
          ...
          >
            <Button
              ...
              variant="outline"
            >
              {i18n.t('SmartSoftwareAccount::Cancel')}
            </Button>
            <Button style={styles.button} onPress={() => findTenant()}>
              {i18n.t('SmartSoftwareAccount::Save')}
            </Button>
          </View>
        </Box>
      ) : null}
