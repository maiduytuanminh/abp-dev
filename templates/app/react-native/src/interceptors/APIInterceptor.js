...
export function initAPIInterceptor(store) {
  ...
  api.interceptors.response.use(
    ...
    error => {
      ...
      if (errorRes) {
        if (errorRes.headers._sserrorformat && errorRes.status === 401) {
...
function showError({ error = {}, status }) {
  let message = '';
  let title = i18n.t('SmartSoftwareAccount::DefaultErrorMessage');
  ...
  if (typeof error === 'string') {
    ...
    switch (status) {
      case 401:
        title = i18n.t('SmartSoftwareAccount::DefaultErrorMessage401');
        message = i18n.t('SmartSoftwareAccount::DefaultErrorMessage401Detail');
        break;
      case 403:
        title = i18n.t('SmartSoftwareAccount::DefaultErrorMessage403');
        message = i18n.t('SmartSoftwareAccount::DefaultErrorMessage403Detail');
        break;
      case 404:
        title = i18n.t('SmartSoftwareAccount::DefaultErrorMessage404');
        message = i18n.t('SmartSoftwareAccount::DefaultErrorMessage404Detail');
        break;
      case 500:
        title = i18n.t('SmartSoftwareAccount::500Message');
        message = i18n.t('SmartSoftwareAccount::InternalServerErrorMessage');
        break;
      default:
        break;
    }
