...
public class IdentityUserAppService_Tests : SmartSoftwareIdentityApplicationTestBase
{
    ...
    public async Task UpdateAsync_Concurrency_Exception()
    {
        ...
        (await Assert.ThrowsAsync<SmartSoftwareIdentityResultException>(async () =>
        {
            await _userAppService.UpdateAsync(johnNash.Id, input);
    ...
    {
        return Guid.NewGuid().ToString("N").Left(16) + "@smartsoftware.io";
    }
