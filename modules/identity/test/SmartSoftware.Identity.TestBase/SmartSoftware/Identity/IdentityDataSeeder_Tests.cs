...
public abstract class IdentityDataSeeder_Tests<TStartupModule> : SmartSoftwareIdentityTestBase<TStartupModule>
    where TStartupModule : ISmartSoftwareModule
{
    ...
    {
        await _identityDataSeeder.SeedAsync("admin@smartsoftware.io", "1q2w3E*");
    ...
    public async Task Should_Create_Admin_User_And_Role_With_TenantId()
    {
        ...
        await _identityDataSeeder.SeedAsync("admin@tenant.smartsoftware.io", "1q2w3E*", tenantId);

        (await _userRepository.FindByNormalizedEmailAsync(_lookupNormalizer.NormalizeEmail("admin@tenant.smartsoftware.io"))).ShouldBeNull();
