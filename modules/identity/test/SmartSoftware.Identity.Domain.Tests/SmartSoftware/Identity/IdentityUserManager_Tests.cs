...
public class IdentityUserManager_Tests : SmartSoftwareIdentityDomainTestBase
{
    ...
    public async Task IdentityUserEmailChangedEto_Test()
    {
        ...
        using (_currentTenant.Change(Guid.NewGuid()))
        {
            ...
            await _identityUserManager.SetEmailAsync(newUser, "newEmail@smartsoftware.io");
            await _distributedEventBus.Received()
                .PublishAsync(
                    Arg.Is<IdentityUserEmailChangedEto>(x =>
                        x.Id == newUser.Id && x.TenantId == newUser.TenantId && x.OldEmail == oldEmail && x.Email == "newEmail@smartsoftware.io"),
                    Arg.Any<bool>(), Arg.Any<bool>());
        }
    ...
    {
        var result = await _identityUserManager.ValidateUserNameAsync("M_y+User-001@smartsoftware.io");
        result.ShouldBeTrue();
    ...
    {
        _identityUserManager.Options.User.AllowedUserNameCharacters = "abcdefghijklmnopqrstuvwxyz0123456789";
        var username = await _identityUserManager.GetUserNameFromEmailAsync("Yönetici@smartsoftware.io");
        ...
        _identityUserManager.Options.User.AllowedUserNameCharacters = "abcdefghijklmnopqrstuvwxyz0123456789";
        username = await _identityUserManager.GetUserNameFromEmailAsync("admin@smartsoftware.io");
    ...
    private static IdentityUser CreateRandomUser()
    {
        return new IdentityUser(
            Guid.NewGuid(),
            Guid.NewGuid().ToString(),
            Guid.NewGuid().ToString() + "@smartsoftware.io"
        );
    }
