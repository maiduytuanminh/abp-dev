...
public class IdentityClaimTypeManager_Tests : SmartSoftwareIdentityDomainTestBase
{
    ...
    {
        await Assert.ThrowsAnyAsync<SmartSoftwareException>(async () => await _claimTypeManager.UpdateAsync(
            new IdentityClaimType(
                Guid.NewGuid(), "Age")));
    }
    ...
    public async Task Static_IdentityClaimType_Cant_Not_Update()
    {
        ...
        await Assert.ThrowsAnyAsync<SmartSoftwareException>(async () => await _claimTypeManager.UpdateAsync(phoneClaim));
    }
}
