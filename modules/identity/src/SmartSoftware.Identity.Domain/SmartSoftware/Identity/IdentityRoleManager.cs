...
public class IdentityRoleManager : RoleManager<IdentityRole>, IDomainService
{
    ...
    public async override Task<IdentityResult> SetRoleNameAsync(IdentityRole role, string name)
    {
        ...
        if (result.Succeeded)
        {
            ...
            await DynamicClaimCache.RemoveManyAsync(userIdList.Select(userId => SmartSoftwareDynamicClaimCacheItem.CalculateCacheKey(userId, role.TenantId)), token: CancellationToken);
        }
