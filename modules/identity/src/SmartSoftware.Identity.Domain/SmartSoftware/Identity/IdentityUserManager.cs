...
public class IdentityUserManager : UserManager<IdentityUser>, IDomainService
{
    ...
    protected async override Task<IdentityResult> UpdateUserAsync(IdentityUser user)
    {
        ...
        if (result.Succeeded)
        {
            await DynamicClaimCache.RemoveAsync(SmartSoftwareDynamicClaimCacheItem.CalculateCacheKey(user.Id, user.TenantId), token: CancellationToken);
        }
    ...
    public virtual async Task ResetRecoveryCodesAsync(IdentityUser user)
    {
        ...
        {
            throw new SmartSoftwareException($"Store is not an instance of {typeof(IdentityUserStore).AssemblyQualifiedName}");
        }
    ...
    public virtual async Task UpdateRoleAsync(Guid sourceRoleId, Guid? targetRoleId)
    {
        ...
        await DynamicClaimCache.RemoveManyAsync(userIdList.Select(userId => SmartSoftwareDynamicClaimCacheItem.CalculateCacheKey(userId, sourceRole.TenantId)), token: CancellationToken);
        ...
        if (targetRole != null)
        {
            ...
            await DynamicClaimCache.RemoveManyAsync(userIdList.Select(userId => SmartSoftwareDynamicClaimCacheItem.CalculateCacheKey(userId, targetRole.TenantId)), token: CancellationToken);
        }
    ...
    public virtual async Task UpdateOrganizationAsync(Guid sourceOrganizationId, Guid? targetOrganizationId)
    {
        ...
        await DynamicClaimCache.RemoveManyAsync(userIdList.Select(userId => SmartSoftwareDynamicClaimCacheItem.CalculateCacheKey(userId, sourceOrganization.TenantId)), token: CancellationToken);
        ...
        if (targetOrganization != null)
        {
            ...
            await DynamicClaimCache.RemoveManyAsync(userIdList.Select(userId => SmartSoftwareDynamicClaimCacheItem.CalculateCacheKey(userId, targetOrganization.TenantId)), token: CancellationToken);
        }
    ...
    public virtual async Task<string> GetUserNameFromEmailAsync(string email)
    {
        ...
        throw new SmartSoftwareIdentityResultException(IdentityResult.Failed(new IdentityErrorDescriber().InvalidUserName(userName)));
    }
}
