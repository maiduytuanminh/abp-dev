$(function (){
    ...
    var commentsService = smartsoftware.cmsKit.admin.comments.commentAdmin;

    var detailsModal = new ss.ModalManager(ss.appPath + "CmsKit/Comments/DetailsModal");
    ...
    var _dataTable = $('#CommentsTable').DataTable(ss.libs.datatables.normalizeConfiguration({
        processing: true,
        serverSide: true,
        paging: true,
        scrollX: true,
        searching: false,
        scrollCollapse: true,
        ajax: ss.libs.datatables.createAjax(commentsService.getList, getFilter),
        columnDefs: [
            {
                width: "10%",
                title: l("Actions"),
                    ...
                    items: [
                        {
                            ...
                            action: function (data) {
                                window.location = ss.appPath + 'CmsKit/Comments/Details/' + data.record.id;
                            }
                        },
                        {
                            text: l('Delete'),
                            visible: ss.auth.isGranted('CmsKit.Comments.Delete'),
