$(function () {
    ...
    var widgetModal = new ss.ModalManager({ viewUrl: ss.appPath + "CmsKit/Contents/AddWidgetModal", modalClass: "addWidgetModal" });
    ...
    function initSelectBlog() {
        ...
        ss.dom.initializers.initializeAutocompleteSelects($selectBlog);
    }
                    ...
                    if (isConfirmed) {
                        ...
                        $formCreate.ajaxSubmit({
                            ...
                            error: function (result) {
                                ss.notify.error(result.responseJSON.error.message);
                                ss.ui.clearBusy();
                            }
                        });
                    }
    ...
    function getUppyHeaders() {
        var headers = {};
        headers[ss.security.antiForgery.tokenHeaderName] = ss.security.antiForgery.getToken();
    ...
    async function submitCoverImage() {
        ...
        if (file) {
            ...
            if (result.failed.length > 0) {
                ss.message.error(l("UploadFailedMessage"));
            } else {
    ...
    $selectBlog.on('change', function () {
        ...
        smartsoftware.cmsKit.blogs.blogFeature
    ...
    function uploadFile(blob, callback, source) {
        ...
        UPPY.upload().then((result) => {
            if (result.failed.length > 0) {
                ss.message.error("File upload failed");
            } else {
