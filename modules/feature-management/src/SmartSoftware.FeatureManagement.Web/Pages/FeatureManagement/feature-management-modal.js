...
(function ($) {
    ...
    let l = ss.localization.getResource("SmartSoftwareFeatureManagement");
    ss.modals.FeatureManagement = function () {
        ss.ResourceLoader.loadScript('/client-proxies/featureManagement-proxy.js');
        $('#ResetToDefaults').click(function (e) {
            ss.message.confirm(l('AreYouSureToResetToDefault'))
                .then(function (confirmed) {
                    if (confirmed) {
                        let providerName = $('#ProviderName').val();
                        let prodiverKey = $('#ProviderKey').val();
                        smartsoftware.featureManagement.features.delete(providerName, prodiverKey).then(function () {
                            ss.notify.success(l('ResetedToDefault'));
                        });
                        setTimeout(function () {
