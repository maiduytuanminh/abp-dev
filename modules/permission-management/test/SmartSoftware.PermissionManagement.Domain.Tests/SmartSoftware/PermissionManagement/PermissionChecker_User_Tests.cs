...
public class PermissionChecker_User_Tests : PermissionTestBase
{
    ...
    private static ClaimsPrincipal CreatePrincipal(Guid? userId, Guid? tenantId = null)
    {
        ...
        if (userId != null)
        {
            claimsIdentity.AddClaim(new Claim(SmartSoftwareClaimTypes.UserId, userId.ToString()));
        }

        if (tenantId != null)
        {
            claimsIdentity.AddClaim(new Claim(SmartSoftwareClaimTypes.TenantId, tenantId.ToString()));
        }
