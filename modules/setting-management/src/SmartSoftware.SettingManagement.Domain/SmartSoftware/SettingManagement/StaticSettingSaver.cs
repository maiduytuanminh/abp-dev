...
public class StaticSettingSaver : IStaticSettingSaver, ITransientDependency
{
    ...
    protected SmartSoftwareSettingOptions SettingOptions { get; }
    ...
    public StaticSettingSaver(
        ...
        IOptions<SmartSoftwareSettingOptions> settingOptions,
    ...
    public async Task SaveAsync()
    {
                         ...
                         TimeSpan.FromMinutes(5)))
        {
            ...
            {
                /* It will re-try */
                throw new SmartSoftwareException("Could not acquire distributed lock for saving static Settings!");
            }
    ...
    {
        return $"{CacheOptions.KeyPrefix}_{ApplicationInfoAccessor.ApplicationName}_SmartSoftwareSettingUpdateLock";
    }

    private string GetCommonDistributedLockKey()
    {
        return $"{CacheOptions.KeyPrefix}_Common_SmartSoftwareSettingUpdateLock";
    }

    private string GetApplicationHashCacheKey()
    {
        return $"{CacheOptions.KeyPrefix}_{ApplicationInfoAccessor.ApplicationName}_SmartSoftwareSettingsHash";
    }

    private string GetCommonStampCacheKey()
    {
        return $"{CacheOptions.KeyPrefix}_SmartSoftwareInMemorySettingCacheStamp";
    }
    ...
    private string CalculateHash(List<SettingDefinitionRecord> settingRecords, IEnumerable<string> deletedSettings)
    {
        var jsonSerializerOptions = new JsonSerializerOptions
        {
            TypeInfoResolver = new DefaultJsonTypeInfoResolver
            {
                Modifiers =
                {
                    new SmartSoftwareIgnorePropertiesModifiers<SettingDefinitionRecord, Guid>().CreateModifyAction(x => x.Id),
                }
            }
        };
