...
public partial class TokenController
{
    ...
    protected IOptions<SmartSoftwareIdentityOptions> SmartSoftwareIdentityOptions => LazyServiceProvider.LazyGetRequiredService<IOptions<SmartSoftwareIdentityOptions>>();
    ...
    protected virtual async Task<IActionResult> HandlePasswordAsync(OpenIddictRequest request)
    {
        using (var scope = ServiceScopeFactory.CreateScope())
        {
            ...
            using (CurrentTenant.Change(tenant?.Id))
            {
                ...
                if (SmartSoftwareIdentityOptions.Value.ExternalLoginProviders.Any())
                {
                    foreach (var externalLoginProviderInfo in SmartSoftwareIdentityOptions.Value.ExternalLoginProviders.Values)
                    {
    ...
    protected virtual async Task<IActionResult> SetSuccessResultAsync(OpenIddictRequest request, IdentityUser user)
    {
        ...
        {
            var claim = new Claim(SmartSoftwareClaimTypes.RememberMe, true.ToString()).SetDestinations(OpenIddictConstants.Destinations.AccessToken);
