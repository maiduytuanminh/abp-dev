...
public static class ServiceCollectionHttpClientProxyExtensions
{
    ...
    /// See <see cref="SmartSoftwareRemoteServiceOptions"/>.
    /// </param>
        ...
        ApplicationServiceTypes applicationServiceTypes = ApplicationServiceTypes.All)
    {
        ...
        foreach (var serviceType in serviceTypes)
        {
            ...
            services.Configure<SmartSoftwareHttpClientOptions>(options =>
            {
        ...
        bool asDefaultService = true)
    {
        ...
        var interceptorAdapterType = typeof(SmartSoftwareAsyncDeterminationInterceptor<>).MakeGenericType(interceptorType);

        var validationInterceptorAdapterType =
            typeof(SmartSoftwareAsyncDeterminationInterceptor<>).MakeGenericType(typeof(ValidationInterceptor));
    ...
    private static IServiceCollection AddHttpClientFactory(
        ...
        [NotNull] string remoteServiceConfigurationName = RemoteServiceConfigurationDictionary.DefaultName)
    {
        var preOptions = services.ExecutePreConfiguredActions<SmartSoftwareHttpClientBuilderOptions>();
        ...
        services.PreConfigure<SmartSoftwareHttpClientBuilderOptions>(options =>
        {
