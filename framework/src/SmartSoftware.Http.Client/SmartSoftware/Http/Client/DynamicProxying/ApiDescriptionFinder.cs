...
public class ApiDescriptionFinder : IApiDescriptionFinder, ITransientDependency
{
    ...
    protected SmartSoftwareCorrelationIdOptions SmartSoftwareCorrelationIdOptions { get; }
    ...
    {
        Cache = cache;
        SmartSoftwareCorrelationIdOptions = ssCorrelationIdOptions.Value;
        ...
        MethodInfo method)
    {
        ...
        throw new SmartSoftwareException($"Could not find remote action for method: {method} on the URL: {baseUrl}");
    }
        ...
        string baseUrl)
    {
        var requestMessage = new HttpRequestMessage(
            HttpMethod.Get,
            baseUrl.EnsureEndsWith('/') + "api/ss/api-definition"
        );
        ...
        {
            throw new SmartSoftwareException("Remote service returns error! StatusCode = " + response.StatusCode);
        }
    ...
    {
        //CorrelationId
        requestMessage.Headers.Add(SmartSoftwareCorrelationIdOptions.HttpHeaderName, CorrelationIdProvider.Get());
