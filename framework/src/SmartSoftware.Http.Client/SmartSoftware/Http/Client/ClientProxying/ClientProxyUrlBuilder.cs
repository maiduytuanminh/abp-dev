...
public class ClientProxyUrlBuilder : ITransientDependency
{
    ...
    protected SmartSoftwareHttpClientProxyingOptions HttpClientProxyingOptions { get; }

    public ClientProxyUrlBuilder(IServiceScopeFactory serviceScopeFactory, IOptions<SmartSoftwareHttpClientProxyingOptions> httpClientProxyingOptions)
    {
    ...
    protected virtual async Task ReplacePathVariablesAsync(StringBuilder urlBuilder, ActionApiDescriptionModel action, IReadOnlyDictionary<string, object> methodArguments, ApiVersionInfo apiVersion)
    {
        ...
        foreach (var pathParameter in pathParameters.Where(p => p.Name != "apiVersion")) //TODO: Constant!
        {
            ...
            if (value == null)
            {
                ...
                {
                    throw new SmartSoftwareException($"Missing path parameter value for {pathParameter.Name} ({pathParameter.NameOnMethod})");
                }
            }
            else
            {
