...
using SmartSoftware.Http.ProxyScripting.Generators;
...
public class ClientProxyBase<TService> : ITransientDependency
{
    ...
    protected IOptions<SmartSoftwareCorrelationIdOptions> SmartSoftwareCorrelationIdOptions => LazyServiceProvider.LazyGetRequiredService<IOptions<SmartSoftwareCorrelationIdOptions>>();
    ...
    protected IOptions<SmartSoftwareHttpClientOptions> ClientOptions => LazyServiceProvider.LazyGetRequiredService<IOptions<SmartSoftwareHttpClientOptions>>();
    ...
    protected virtual ClientProxyRequestContext BuildHttpProxyClientProxyContext(string methodName, ClientProxyRequestTypeValue? arguments = null)
    {
        ...
        if (action == null)
        {
            throw new SmartSoftwareException($"The API description of the {typeof(TService).FullName}.{methodName} method was not found!");
        }
    ...
    {
        var clientConfig = ClientOptions.Value.HttpClientProxies.GetOrDefault(requestContext.ServiceType) ?? throw new SmartSoftwareException($"Could not get HttpClientProxyConfig for {requestContext.ServiceType.FullName}.");
        ...
        {
            throw new SmartSoftwareRemoteCallException($"An error occurred during the SS remote HTTP request. ({ex.Message}) See the inner exception for details.", ex);
        }
    ...
    protected virtual async Task<string?> GetConfiguredApiVersionAsync(ClientProxyRequestContext requestContext)
    {
        var clientConfig = ClientOptions.Value.HttpClientProxies.GetOrDefault(requestContext.ServiceType)
                           ?? throw new SmartSoftwareException($"Could not get DynamicHttpClientProxyConfig for {requestContext.ServiceType.FullName}.");
    ...
    protected virtual async Task ThrowExceptionForResponseAsync(HttpResponseMessage response)
    {
        ...
        if (response.Headers.Contains(SmartSoftwareHttpConsts.SmartSoftwareErrorFormat))
        {
            ...
            {
                throw new SmartSoftwareRemoteCallException(
            ...
            throw new SmartSoftwareRemoteCallException(errorResponse.Error)
            {
                HttpStatusCode = (int)response.StatusCode
            };
        }
        ...
        ApiVersionInfo apiVersion)
    {
        ...
        //CorrelationId
        requestMessage.Headers.Add(SmartSoftwareCorrelationIdOptions.Value.HttpHeaderName, CorrelationIdProvider.Get());
