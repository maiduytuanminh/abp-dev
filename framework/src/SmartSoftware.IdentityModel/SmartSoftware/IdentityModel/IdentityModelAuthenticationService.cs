...
public class IdentityModelAuthenticationService : IIdentityModelAuthenticationService, ITransientDependency
{
    ...
    protected SmartSoftwareIdentityClientOptions ClientOptions { get; }
    ...
    protected ISmartSoftwareHostEnvironment SmartSoftwareHostEnvironment { get; }

    public IdentityModelAuthenticationService(
        IOptions<SmartSoftwareIdentityClientOptions> options,
        ...
        ISmartSoftwareHostEnvironment ssHostEnvironment)
    {
        ...
        SmartSoftwareHostEnvironment = ssHostEnvironment;
    ...
    public virtual async Task<string> GetAccessTokenAsync(IdentityClientConfiguration configuration)
    {
        ...
        if (tokenCacheItem == null)
        {
            ...
            if (tokenResponse.IsError)
            {
                ...
                {
                    throw new SmartSoftwareException($"Could not get token from the OpenId Connect server! ErrorType: {tokenResponse.ErrorType}. " +
                ...
                throw new SmartSoftwareException(withoutInnerException[0]);
            }
            ...
            {
                AbsoluteExpirationRelativeToNow = SmartSoftwareHostEnvironment.IsDevelopment()
    ...
    protected virtual async Task<IdentityModelDiscoveryDocumentCacheItem> GetDiscoveryResponse(IdentityClientConfiguration configuration)
    {
        ...
        if (discoveryDocumentCacheItem == null)
        {
            ...
            {
                throw new SmartSoftwareException($"Could not retrieve the OpenId Connect discovery document! " +
    ...
    protected virtual async Task<TokenResponse> GetTokenResponse(IdentityClientConfiguration configuration)
    {
        using (var httpClient = HttpClientFactory.CreateClient(HttpClientName))
        {
            ...
            switch (configuration.GrantType)
            {
                ...
                default:
                    throw new SmartSoftwareException("Grant type was not implemented: " + configuration.GrantType);
            }
        }
    }
    ...
    protected virtual async Task<TokenResponse> RequestDeviceAuthorizationAsync(HttpClient httpClient, IdentityClientConfiguration configuration)
    {
        ...
        if (response.IsError)
        {
            throw new SmartSoftwareException(response.ErrorDescription);
        }
        ...
        for (var i = 0; i < ((response.ExpiresIn ?? 300) / response.Interval + 1); i++)
        {
            ...
            if (tokenResponse.IsError)
            {
                switch (tokenResponse.Error)
                {
                    ...
                    case "expired_token":
                        throw new SmartSoftwareException("This 'device_code' has expired. (expired_token)");

                    case "access_denied":
                        throw new SmartSoftwareException("User denies the request(access_denied)");
                }
            }
        ...
        throw new SmartSoftwareException("Timeout!");
    }
