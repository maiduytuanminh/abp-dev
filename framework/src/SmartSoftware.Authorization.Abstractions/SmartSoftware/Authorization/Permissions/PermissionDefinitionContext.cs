...
public class PermissionDefinitionContext : IPermissionDefinitionContext
{
        ...
        ILocalizableString? displayName = null)
    {
        ...
        {
            throw new SmartSoftwareException($"There is already an existing permission group with name: {name}");
        }
    ...
    public virtual PermissionGroupDefinition GetGroup([NotNull] string name)
    {
        ...
        if (group == null)
        {
            throw new SmartSoftwareException($"Could not find a permission definition group with the given name: {name}");
        }
    ...
    public virtual void RemoveGroup(string name)
    {
        ...
        if (!Groups.ContainsKey(name))
        {
            throw new SmartSoftwareException($"Not found permission group with name: {name}");
        }
