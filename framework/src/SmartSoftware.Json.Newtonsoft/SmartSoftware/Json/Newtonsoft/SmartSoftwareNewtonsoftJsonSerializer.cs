...
public class SmartSoftwareNewtonsoftJsonSerializer : IJsonSerializer, ITransientDependency
{
    protected IRootServiceProvider RootServiceProvider { get; }
    protected IOptions<SmartSoftwareNewtonsoftJsonSerializerOptions> Options { get; }

    public SmartSoftwareNewtonsoftJsonSerializer(IRootServiceProvider rootServiceProvider, IOptions<SmartSoftwareNewtonsoftJsonSerializerOptions> options)
    {
    ...
    protected virtual JsonSerializerSettings CreateJsonSerializerOptions(bool camelCase = true, bool indented = false)
    {
        ...
        }, _ =>
        {
            ...
            if (!camelCase)
            {
                //Default contract resolver is SmartSoftwareCamelCasePropertyNamesContractResolver}
                settings.ContractResolver = new SmartSoftwareDefaultContractResolver(RootServiceProvider.GetRequiredService<SmartSoftwareDateTimeConverter>());
            }
