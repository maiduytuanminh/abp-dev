...
using SmartSoftware.BlazoriseUI.Components;
...
public abstract class SmartSoftwareCrudPageBase<
        TAppService,
        TEntityDto,
        TKey>
    : SmartSoftwareCrudPageBase<
        TAppService,
        TEntityDto,
        TKey,
    ...
    where TUpdateViewModel : class, new()
{
    ...
    [Inject] protected IStringLocalizer<SmartSoftwareUiResource> UiLocalizer { get; set; } = default!;
    [Inject] public ISmartSoftwareEnumLocalizer SmartSoftwareEnumLocalizer { get; set; } = default!;
    ...
    protected SmartSoftwareCrudPageBase()
    {
    ...
    /// Throws <see cref="SmartSoftwareAuthorizationException"/> if given policy was not granted for the current user.
    ///
                ...
                else
                {
                    ...
                    if (propertyInfo.Type.IsEnum)
                    {
                        column.ValueConverter = (val) =>
                            SmartSoftwareEnumLocalizer.GetString(propertyInfo.Type, val.As<ExtensibleObject>().ExtraProperties[propertyInfo.Name]!, new IStringLocalizer?[]{ StringLocalizerFactory.CreateDefaultOrNull() });
                    }
