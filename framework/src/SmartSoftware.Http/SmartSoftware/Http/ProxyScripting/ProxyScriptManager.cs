...
public class ProxyScriptManager : IProxyScriptManager, ITransientDependency
{
    ...
    private readonly SmartSoftwareApiProxyScriptingOptions _options;
    ...
    public ProxyScriptManager(
        ...
        IOptions<SmartSoftwareApiProxyScriptingOptions> options)
    {
    ...
    private string CreateScript(ProxyScriptingModel scriptingModel)
    {
        ...
        {
            throw new SmartSoftwareException($"Could not find a proxy script generator with given name: {scriptingModel.GeneratorType}");
        }
