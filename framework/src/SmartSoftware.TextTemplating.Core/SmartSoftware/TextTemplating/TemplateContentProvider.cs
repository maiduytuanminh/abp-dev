...
public class TemplateContentProvider : ITemplateContentProvider, ITransientDependency
{
    ...
    public SmartSoftwareTextTemplatingOptions Options { get; }
        ...
        bool useCurrentCultureIfCultureNameIsNull = true)
    {
        ...
        if (!Options.ContentContributors.Any())
        {
            throw new SmartSoftwareException(
                $"No template content contributor was registered. Use {nameof(SmartSoftwareTextTemplatingOptions)} to register contributors!"
            );
        }
