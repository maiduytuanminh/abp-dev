...
public class StaticTemplateDefinitionStore : IStaticTemplateDefinitionStore, ISingletonDependency
{
    ...
    public StaticTemplateDefinitionStore(IOptions<SmartSoftwareTextTemplatingOptions> options, IServiceProvider serviceProvider)
    {
    ...
    public virtual Task<TemplateDefinition> GetAsync(string name)
    {
        ...
        if (template == null)
        {
            throw new SmartSoftwareException("Undefined template: " + name);
        }
