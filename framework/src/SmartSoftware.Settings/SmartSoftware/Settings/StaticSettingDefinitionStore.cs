...
public class StaticSettingDefinitionStore : IStaticSettingDefinitionStore, ISingletonDependency
{
    ...
    public StaticSettingDefinitionStore(IOptions<SmartSoftwareSettingOptions> options, IServiceProvider serviceProvider)
    {
    ...
    public virtual async Task<SettingDefinition> GetAsync(string name)
    {
        ...
        if (setting == null)
        {
            throw new SmartSoftwareException("Undefined setting: " + name);
        }
