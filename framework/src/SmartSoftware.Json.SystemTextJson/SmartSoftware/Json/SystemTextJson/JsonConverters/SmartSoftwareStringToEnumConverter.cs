...
public class SmartSoftwareStringToEnumConverter<T> : JsonConverter<T>
    where T : struct, Enum
{
    ...
    public SmartSoftwareStringToEnumConverter()
        : this(namingPolicy: null, allowIntegerValues: true)
    {

    }

    public SmartSoftwareStringToEnumConverter(JsonNamingPolicy? namingPolicy = null, bool allowIntegerValues = true)
    {
    ...
    public override T Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        _readJsonSerializerOptions ??= JsonSerializerOptionsHelper.Create(options, x =>
                x == this ||
                x.GetType() == typeof(SmartSoftwareStringToEnumFactory),
