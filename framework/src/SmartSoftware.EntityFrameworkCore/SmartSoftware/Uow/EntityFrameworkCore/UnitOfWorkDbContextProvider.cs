    ...
    where TDbContext : IEfCoreDbContext
{
    ...
    public virtual TDbContext GetDbContext()
    {
        ...
        if (unitOfWork == null)
        {
            throw new SmartSoftwareException("A DbContext can only be created inside a unit of work!");
        }
    ...
    protected virtual TDbContext CreateDbContext(IUnitOfWork unitOfWork, string connectionStringName, string connectionString)
    {
        ...
        using (DbContextCreationContext.Use(creationContext))
        {
            ...
            if (dbContext is ISmartSoftwareEfCoreDbContext ssEfCoreDbContext)
            {
                ssEfCoreDbContext.Initialize(
                    new SmartSoftwareEfCoreDbContextInitializationContext(
                        unitOfWork
                    )
                );
            }
