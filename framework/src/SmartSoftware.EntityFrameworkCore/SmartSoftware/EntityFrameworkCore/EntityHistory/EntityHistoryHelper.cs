...
public class EntityHistoryHelper : IEntityHistoryHelper, ITransientDependency
{
    ...
    protected SmartSoftwareEfCoreNavigationHelper? SmartSoftwareEfCoreNavigationHelper { get; set; }
    ...
    {
        SmartSoftwareEfCoreNavigationHelper = ssEfCoreNavigationHelper;
    }
    ...
    protected virtual DateTime GetChangeTime(EntityChangeInfo entityChange)
    {
        ...
        switch (entityChange.ChangeType)
        {
            ...
            default:
                throw new SmartSoftwareException($"Unknown {nameof(EntityChangeInfo)}: {entityChange}");
        }
    }
    ...
    protected virtual string? GetEntityId(object entityAsObj)
    {
        ...
        throw new SmartSoftwareException($"Entities should implement the {typeof(IEntity).AssemblyQualifiedName} interface or {typeof(ValueObject).AssemblyQualifiedName} class! " +
    ...
    protected virtual List<EntityPropertyChangeInfo> GetPropertyChanges(EntityEntry entityEntry)
    {
        ...
        if (Options.SaveEntityHistoryWhenNavigationChanges && SmartSoftwareEfCoreNavigationHelper != null)
        {
            foreach (var (navigationEntry, index) in entityEntry.Navigations.Select((value, i) => ( value, i )))
            {
                if (SmartSoftwareEfCoreNavigationHelper.IsNavigationEntryModified(entityEntry, index))
                {
                    propertyChanges.Add(new EntityPropertyChangeInfo
                    {
    ...
    protected virtual bool HasNavigationPropertiesChanged(EntityEntry entityEntry)
    {
        return Options.SaveEntityHistoryWhenNavigationChanges &&
               SmartSoftwareEfCoreNavigationHelper != null &&
               SmartSoftwareEfCoreNavigationHelper.IsEntityEntryModified(entityEntry);
    }
