...
using SmartSoftware.EntityFrameworkCore.EntityHistory;
using SmartSoftware.EntityFrameworkCore.Modeling;
using SmartSoftware.EntityFrameworkCore.ValueConverters;
...
public abstract class SmartSoftwareDbContext<TDbContext> : DbContext, ISmartSoftwareEfCoreDbContext, ITransientDependency
    where TDbContext : DbContext
{
    ...
    public IOptions<SmartSoftwareEntityChangeOptions> EntityChangeOptions => LazyServiceProvider.LazyGetRequiredService<IOptions<SmartSoftwareEntityChangeOptions>>();
    ...
    public ILogger<SmartSoftwareDbContext<TDbContext>> Logger => LazyServiceProvider.LazyGetService<ILogger<SmartSoftwareDbContext<TDbContext>>>(NullLogger<SmartSoftwareDbContext<TDbContext>>.Instance);

    public SmartSoftwareEfCoreNavigationHelper SmartSoftwareEfCoreNavigationHelper => LazyServiceProvider.LazyGetRequiredService<SmartSoftwareEfCoreNavigationHelper>();

    public IOptions<SmartSoftwareDbContextOptions> Options => LazyServiceProvider.LazyGetRequiredService<IOptions<SmartSoftwareDbContextOptions>>();

    private static readonly MethodInfo ConfigureBasePropertiesMethodInfo
        = typeof(SmartSoftwareDbContext<TDbContext>)
            .GetMethod(
    ...
    protected SmartSoftwareDbContext(DbContextOptions<TDbContext> options)
        : base(options)
    {
    }
    ...
    public override async Task<int> SaveChangesAsync(bool acceptAllChangesOnSuccess, CancellationToken cancellationToken = default)
    {
        try
        {
            foreach (var entityEntry in SmartSoftwareEfCoreNavigationHelper.GetChangedEntityEntries())
            {
            ...
            if (auditLog != null)
            {
                EntityHistoryHelper.InitializeNavigationHelper(SmartSoftwareEfCoreNavigationHelper);
        ...
        catch (DbUpdateConcurrencyException ex)
        {
            ...
            throw new SmartSoftwareDbConcurrencyException(ex.Message, ex);
        }
        finally
        {
            ChangeTracker.AutoDetectChangesEnabled = true;
            SmartSoftwareEfCoreNavigationHelper.Clear();
        }
    }
    ...
    /// <summary>
    /// This method will call the DbContext <see cref="SaveChangesAsync(bool, CancellationToken)"/> method directly of EF Core, which doesn't apply concepts of ss.
    /// </summary>
    ...
    public virtual void Initialize(SmartSoftwareEfCoreDbContextInitializationContext initializationContext)
    {
    ...
    {
        SmartSoftwareEfCoreNavigationHelper.ChangeTracker_Tracked(sender, e);
    ...
    {
        SmartSoftwareEfCoreNavigationHelper.ChangeTracker_StateChanged(sender, e);
        PublishEventsForTrackedEntity(e.Entry);
    }
    ...
    protected virtual void PublishEventsForTrackedEntity(EntityEntry entry)
    {
        switch (entry.State)
        {
            case EntityState.Added:
                ApplySmartSoftwareConceptsForAddedEntity(entry);
                EntityChangeEventHelper.PublishEntityCreatedEvent(entry.Entity);
                break;

            case EntityState.Modified:
                ApplySmartSoftwareConceptsForModifiedEntity(entry);
                ...
                else if (EntityChangeOptions.Value.PublishEntityUpdatedEventWhenNavigationChanges && SmartSoftwareEfCoreNavigationHelper.IsEntityEntryModified(entry))
                {
                    if (entry.Entity is ISoftDelete && entry.Entity.As<ISoftDelete>().IsDeleted)
                    {
                        EntityChangeEventHelper.PublishEntityDeletedEvent(entry.Entity);
            ...
            case EntityState.Deleted:
                ApplySmartSoftwareConceptsForDeletedEntity(entry);
                EntityChangeEventHelper.PublishEntityDeletedEvent(entry.Entity);
                break;
        }
    }
    ...
    protected virtual void ApplySmartSoftwareConceptsForAddedEntity(EntityEntry entry)
    {
        CheckAndSetId(entry);
    ...
    protected virtual void ApplySmartSoftwareConceptsForModifiedEntity(EntityEntry entry)
    {
    ...
    protected virtual void ApplySmartSoftwareConceptsForDeletedEntity(EntityEntry entry)
    {
        ...
        where TEntity : class
    {
        if (mutableEntityType.BaseType == null && ShouldFilterEntity<TEntity>(mutableEntityType))
        {
            ...
            {
                modelBuilder.Entity<TEntity>().HasSmartSoftwareQueryFilter(filterExpression);
            }
        }
    }
                                           ...
                                           ReflectionHelper.GetSingleAttributeOfMemberOrDeclaringTypeOrDefault<DisableDateTimeNormalizationAttribute>(property.PropertyInfo) == null))
            {
				modelBuilder
                    ...
                    .HasConversion(property.ClrType == typeof(DateTime)
                        ? new SmartSoftwareDateTimeValueConverter(Clock)
                        : new SmartSoftwareNullableDateTimeValueConverter(Clock));
            }
        }
    }
