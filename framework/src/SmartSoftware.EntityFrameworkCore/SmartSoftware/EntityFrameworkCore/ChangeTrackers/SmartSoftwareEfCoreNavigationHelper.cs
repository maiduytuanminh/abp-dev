...
/// </summary>
public class SmartSoftwareEfCoreNavigationHelper : ITransientDependency
{
    protected Dictionary<string, SmartSoftwareEntityEntry> EntityEntries { get; } = new();
    ...
    protected virtual void EntityEntryTrackedOrStateChanged(EntityEntry entityEntry)
    {
        ...
        EntityEntries.Add(entryId, new SmartSoftwareEntityEntry(entryId, entityEntry));
    }
    ...
    protected virtual void RecursiveDetectChanges(EntityEntry entityEntry)
    {
        ...
        foreach (var foreignKey in foreignKeys)
        {
            ...
            if (entryId == null || !EntityEntries.TryGetValue(entryId, out var ssEntityEntry))
            {
                continue;
            }

            if (!ssEntityEntry.IsModified)
            {
                ssEntityEntry.IsModified = true;
                RecursiveDetectChanges(ssEntityEntry.EntityEntry);
            }

            var navigationEntry = ssEntityEntry.NavigationEntries.FirstOrDefault(x => x.NavigationEntry.Metadata is INavigation navigationMetadata && navigationMetadata.ForeignKey == foreignKey) ??
                                  ssEntityEntry.NavigationEntries.FirstOrDefault(x => x.NavigationEntry.Metadata is ISkipNavigation skipNavigationMetadata && skipNavigationMetadata.ForeignKey == foreignKey);
        ...
        foreach (var skipNavigation in skipNavigations)
        {
            ...
            foreach (var joinEntry in stateManager.Entries)
            {
                ...
                var navigationEntry = ssEntityEntry.NavigationEntries.FirstOrDefault(x => x.NavigationEntry.Metadata is INavigation navigationMetadata && navigationMetadata.ForeignKey == inverseForeignKey) ??
                                      ssEntityEntry.NavigationEntries.FirstOrDefault(x => x.NavigationEntry.Metadata is ISkipNavigation skipNavigationMetadata && skipNavigationMetadata.ForeignKey == inverseForeignKey);
                if (navigationEntry != null)
                {
                    navigationEntry.IsModified = true;
                }
...
#pragma warning restore EF1001
    ...
    public virtual bool IsEntityEntryModified(EntityEntry entityEntry)
    {
        ...
        return EntityEntries.TryGetValue(entryId, out var ssEntityEntry) && ssEntityEntry.IsModified;
    }
    ...
    public virtual bool IsNavigationEntryModified(EntityEntry entityEntry,  int navigationEntryIndex)
    {
        ...
        if (!EntityEntries.TryGetValue(entryId, out var ssEntityEntry))
        {
            return false;
        }

        var navigationEntryProperty = ssEntityEntry.NavigationEntries.ElementAtOrDefault(navigationEntryIndex);
