...
namespace SmartSoftware.AspNetCore.Mvc.Dapr.EventBus.Controllers;

[Area("ss")]
[RemoteService(Name = "ss")]
public class SmartSoftwareAspNetCoreMvcDaprEventsController : SmartSoftwareController
{
    [HttpPost(SmartSoftwareAspNetCoreMvcDaprPubSubConsts.DaprEventCallbackUrl)]
    public virtual async Task<IActionResult> EventAsync()
    {
        ...
        if (IsSmartSoftwareDaprEventData(data))
        {
            var daprEventData = daprSerializer.Deserialize(data, typeof(SmartSoftwareDaprEventData)).As<SmartSoftwareDaprEventData>();
    ...
    protected  virtual bool IsSmartSoftwareDaprEventData(string data)
    {
