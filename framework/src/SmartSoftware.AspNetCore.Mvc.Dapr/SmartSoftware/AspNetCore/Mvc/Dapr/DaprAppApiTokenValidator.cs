...
public class DaprAppApiTokenValidator : IDaprAppApiTokenValidator, ISingletonDependency
{
    ...
    public virtual void CheckDaprAppApiToken()
    {
        ...
        {
            throw new SmartSoftwareAuthorizationException("Expected Dapr App API Token is not provided! Dapr should set the 'dapr-api-token' HTTP header.");
        }

        if (expectedAppApiToken != headerAppApiToken)
        {
            throw new SmartSoftwareAuthorizationException("The Dapr App API Token (provided in the 'dapr-api-token' HTTP header) doesn't match the expected value!");
        }
    }
    ...
    {
        return HttpContextAccessor.HttpContext ?? throw new SmartSoftwareException("HttpContext is not available!");
    }
}
