...
public abstract class EmailSenderConfiguration : IEmailSenderConfiguration
{
    ...
    /// <summary>
    /// Gets a setting value by checking. Throws <see cref="SmartSoftwareException"/> if it's null or empty.
    /// </summary>
    ...
    protected async Task<string> GetNotEmptySettingValueAsync(string name)
    {
        ...
        {
            throw new SmartSoftwareException($"Setting value for '{name}' is null or empty!");
        }
