...
public static class SmartSoftwareRegistrationBuilderExtensions
{
    public static IRegistrationBuilder<TLimit, TActivatorData, TRegistrationStyle> ConfigureSmartSoftwareConventions<TLimit, TActivatorData, TRegistrationStyle>(
        ...
        where TActivatorData : ReflectionActivatorData
    {
        ...
        {
            var disableSmartSoftwareFeaturesAttribute = serviceRegistredArgs.ImplementationType.GetCustomAttribute<DisableSmartSoftwareFeaturesAttribute>(true);
            if (disableSmartSoftwareFeaturesAttribute == null || !disableSmartSoftwareFeaturesAttribute.DisableInterceptors)
            {
    ...
    private static IRegistrationBuilder<TLimit, TActivatorData, TRegistrationStyle> EnablePropertyInjection<TLimit, TActivatorData, TRegistrationStyle>(
        ...
        where TActivatorData : ReflectionActivatorData
    {
        // Enable Property Injection only for types in an assembly containing an SmartSoftwareModule and without a DisablePropertyInjection attribute on class or properties.
        ...
        {
            registrationBuilder = registrationBuilder.PropertiesAutowired(new SmartSoftwarePropertySelector(false));
        }
        ...
        where TActivatorData : ReflectionActivatorData
    {
        ...
        foreach (var interceptor in interceptors)
        {
            registrationBuilder.InterceptedBy(
                typeof(SmartSoftwareAsyncDeterminationInterceptor<>).MakeGenericType(interceptor)
            );
        }
