...
namespace SmartSoftware.AspNetCore.SignalR.Authentication;

public class SmartSoftwareAuthenticationHubFilter : IHubFilter
{
    ...
    protected virtual async Task HandleDynamicClaimsPrincipalAsync(ClaimsPrincipal? claimsPrincipal, IServiceProvider serviceProvider, HubCallerContext hubCallerContext, bool skipCheckDynamicClaimsInterval)
    {
        if (claimsPrincipal?.Identity != null &&
            ...
            serviceProvider.GetRequiredService<IOptions<SmartSoftwareClaimsPrincipalFactoryOptions>>().Value
                .IsDynamicClaimsEnabled)
        {
            var checkDynamicClaimsInterval = serviceProvider.GetRequiredService<IOptions<SmartSoftwareSignalROptions>>().Value.CheckDynamicClaimsInterval;
            ...
            claimsPrincipal = await serviceProvider.GetRequiredService<ISmartSoftwareClaimsPrincipalFactory>()
