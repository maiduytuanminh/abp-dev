...
public class RemoteServiceExceptionHandler : IRemoteServiceExceptionHandler, ITransientDependency
{
    ...
    public async Task EnsureSuccessfulHttpResponseAsync(HttpResponseMessage responseMessage)
    {
        ...
        var remoteServiceErrorMessage = await GetSmartSoftwareRemoteServiceErrorAsync(responseMessage);
    ...
    public async Task<string> GetSmartSoftwareRemoteServiceErrorAsync(HttpResponseMessage responseMessage)
    {
