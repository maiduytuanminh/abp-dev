...
using SmartSoftware.Cli.GitHub;
...
public class SmartSoftwareIoSourceCodeStore : ISourceCodeStore, ITransientDependency
{
    public ILogger<SmartSoftwareIoSourceCodeStore> Logger { get; set; }
    ...
    public SmartSoftwareIoSourceCodeStore(
        ...
        CliVersionService cliVersionService)
    {
        ...
        Logger = NullLogger<SmartSoftwareIoSourceCodeStore>.Instance;
    }
        ...
        bool trustUserVersion = false)
    {
        ...
        if (version == null)
        {
            if (latestVersion == null)
            {
                ...
                Logger.LogWarning(string.Empty);
                throw new CliUsageException("Use command: ss new Acme.BookStore -v version");
            }
        ...
        if (type == SourceCodeTypes.Template)
        {
            ...
            if (outputWarning)
            {
                ...
                Logger.LogWarning($"> dotnet tool uninstall -g smartsoftware.cli");
                Logger.LogWarning(!templateVersion.IsPrerelease
                    ? $"> dotnet tool install -g smartsoftware.cli --version \"{templateVersion.Major}.{templateVersion.Minor}.*\""
                    : $"> dotnet tool install -g smartsoftware.cli --version {templateVersion}");
        ...
        bool includePreReleases = false)
    {
        if (url == null)
        {
            url = $"{CliUrls.WwwSmartSoftwareIo}api/download/{type}/get-version/";
        }
    ...
    private async Task<string> GetTemplateNugetVersionAsync(string name, string type, string version)
    {
        ...
        try
        {
            var url = $"{CliUrls.WwwSmartSoftwareIo}api/download/{type}/get-nuget-version/";
            var client = _cliHttpClientFactory.CreateClient();
    ...
    {
        var url = $"{CliUrls.WwwSmartSoftwareIo}api/download/all-versions?includePreReleases=true";
    ...
    {
        var url = $"{CliUrls.WwwSmartSoftwareIo}api/download/{input.Type}/";
