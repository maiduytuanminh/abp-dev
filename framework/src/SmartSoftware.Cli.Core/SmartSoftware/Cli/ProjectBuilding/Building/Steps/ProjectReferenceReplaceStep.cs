...
public class ProjectReferenceReplaceStep : ProjectBuildPipelineStep
{
    public override void Execute(ProjectBuildContext context)
    {
        ...
        {
            var localSmartSoftwareRepoPath = context.BuildArgs.SmartSoftwareGitHubLocalRepositoryPath;

            if (string.IsNullOrWhiteSpace(localSmartSoftwareRepoPath))
            {
                return;
            }
    ...
    private abstract class ProjectReferenceReplacer
    {
        ...
        {
            private readonly string _gitHubSmartSoftwareLocalRepositoryPath;

            public LocalProjectPathReferenceReplacer(ProjectBuildContext context, string projectName, string gitHubSmartSoftwareLocalRepositoryPath)
                : base(context, projectName)
            {
                _gitHubSmartSoftwareLocalRepositoryPath = gitHubSmartSoftwareLocalRepositoryPath;
            }
            ...
            private string SetGithubPath(string includeValue)
            {
                ...
                if (!string.IsNullOrWhiteSpace(_gitHubSmartSoftwareLocalRepositoryPath))
                {
                    if (includeValue.StartsWith("ss\\", StringComparison.InvariantCultureIgnoreCase))
                    {
                        return _gitHubSmartSoftwareLocalRepositoryPath.EnsureEndsWith('\\') + includeValue.Substring("ss\\".Length);
                    }

                    return _gitHubSmartSoftwareLocalRepositoryPath.EnsureEndsWith('\\') + "ss\\" + includeValue;
                }

                return _gitHubSmartSoftwareLocalRepositoryPath.EnsureEndsWith('\\') + includeValue;
            }
        }
    }
}
