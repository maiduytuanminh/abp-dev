...
public class RemoveProjectFromSolutionStep : ProjectBuildPipelineStep
{
    ...
    public override void Execute(ProjectBuildContext context)
    {
        ...
        RemoveProjectFromSmartSoftwaremdlFile(context);
    }

    private void RemoveProjectFromSmartSoftwaremdlFile(ProjectBuildContext context)
    {
        var ssmdlFile = context.FindFile(_solutionFilePath.RemovePostFix(".sln") + ".ssmdl");
        ...
        var jsonRoot = JObject.Parse(ssmdlFile.Content);
        var packagesObj = (JObject)jsonRoot["packages"];

        packagesObj?.Remove(_projectName);

        ssmdlFile.SetContent(jsonRoot.ToString(Formatting.Indented));
    }
