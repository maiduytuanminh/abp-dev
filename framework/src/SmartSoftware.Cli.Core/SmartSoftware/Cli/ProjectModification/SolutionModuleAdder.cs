...
using SmartSoftware.Cli.ProjectModification.Events;
...
public class SolutionModuleAdder : ITransientDependency
{
        ...
        bool newProTemplate = false)
    {
        ...
        {
            await SetLeptonXSmartSoftwareVersionsAsync(solutionFile, Path.Combine(modulesFolderInSolution, module.Name));
        }
    ...
    private async Task SetLeptonXSmartSoftwareVersionsAsync(string solutionFile, string combine)
    {
        var ssVersion = SolutionPackageVersionFinder.FindByCsprojVersion(solutionFile);
        ...
        foreach (var project in projects)
        {
            File.WriteAllText(project,
                File.ReadAllText(project).Replace("\"$(SmartSoftwareVersion)\"", $"\"{ssVersion}\"")
            );
        }
    }
        ...
        NuGetPackageTarget target, bool isTieredProject)
    {
        ...
        {
            if (target == NuGetPackageTarget.Web && package.Name.StartsWith("SmartSoftware.Account"))
            {
                continue;
            }
    ...
    protected async Task ModifyDbContext(string[] projectFiles, ModuleInfo module, bool skipDbMigrations = false)
    {
        ...
        var dbContextFile = DerivedClassFinder.Find(dbMigrationsProject, "SmartSoftwareDbContext").FirstOrDefault();

        if (dbContextFile == null)
        {
            Logger.LogDebug(
                $"{dbMigrationsProject} project doesn't have a class that is derived from \"SmartSoftwareDbContext\".");
            return;
        }
        ...
        bool newProTemplate = false)
    {
        ...
        var url = $"{CliUrls.WwwSmartSoftwareIo}api/app/module/byNameWithDetails/?name=" + moduleName;
