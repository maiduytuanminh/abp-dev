...
public class NpmPackagesUpdater : ITransientDependency
{
    ...
    private async Task CreateNpmrcFileAsync(string directoryName)
    {
        ...
        if (await NpmrcFileExistAsync(directoryName))
        {
            ...
            if (!fileContent.Contains(ssRegistry))
            {
                fileContent += Environment.NewLine + ssRegistry;
            }

            if (!fileContent.Contains(smartsoftwareRegistry))
            {
                fileContent += Environment.NewLine + smartsoftwareRegistry;
            }

            if (!fileContent.Contains(smartsoftwareRegistry))
            {
                fileContent += smartsoftwareRegistry;
            }
        ...
        sw.WriteLine(ssRegistry);
        sw.WriteLine(smartsoftwareRegistry);
        sw.WriteLine(smartsoftwareRegistry);
    }
        ...
        bool includePreRc = false)
    {
        ...
        var ssPackages = GetSmartSoftwarePackagesFromPackageJson(packageJson);
        ...
        foreach (var ssPackage in ssPackages)
        {
            var updated = await TryUpdatingPackage(filePath, ssPackage, includePreviews, includeReleaseCandidates,
    ...
    protected virtual List<JProperty> GetSmartSoftwarePackagesFromPackageJson(JObject fileObject)
    {
        var dependencyList = new[] { "dependencies", "devDependencies", "peerDependencies" };
        var ssPackages = new List<JProperty>();
        ...
        foreach (var dependencyListName in dependencyList)
        {
            ...
            ssPackages
                .AddRange(
                properties.Where(
                      p => p.Name.StartsWith("@smartsoftware/")
                        || p.Name.StartsWith("@smartsoftware/")
                        || p.Name.StartsWith("@smartsoftware/")).ToList()
                );
        }
