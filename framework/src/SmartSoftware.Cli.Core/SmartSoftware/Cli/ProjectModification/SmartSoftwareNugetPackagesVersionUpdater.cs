...
public class SmartSoftwareNugetPackagesVersionUpdater : ITransientDependency
{
    ...
    public ILogger<SmartSoftwareNugetPackagesVersionUpdater> Logger { get; set; }
    public static Encoding DefaultEncoding = Encoding.UTF8;

    public SmartSoftwareNugetPackagesVersionUpdater(PackageVersionCheckerService packageVersionCheckerService, MyGetPackageListFinder myGetPackageListFinder)
    {
        _packageVersionCheckerService = packageVersionCheckerService;
        _myGetPackageListFinder = myGetPackageListFinder;
        Logger = NullLogger<SmartSoftwareNugetPackagesVersionUpdater>.Instance;
    }
    ...
    public async Task UpdateSolutionAsync(string solutionPath, bool includePreviews = false, bool includeReleaseCandidates = false, bool switchToStable = false, bool checkAll = false, string version = null)
    {
        ...
        {
            var latestVersionInfo = await _packageVersionCheckerService.GetLatestVersionOrNullAsync("SmartSoftware.Core", includeReleaseCandidates: includeReleaseCandidates);
            var latestReleaseCandidateVersionInfo = await _packageVersionCheckerService.GetLatestVersionOrNullAsync("SmartSoftware.Core", includeReleaseCandidates: true);
            var latestVersionFromMyGet = await GetLatestVersionFromMyGet("SmartSoftware.Core");
            ...
            async Task UpdateAsync(string filePath)
            {
                using (var fs = File.Open(filePath, FileMode.Open, FileAccess.ReadWrite, FileShare.None))
                {
                    using (var sr = new StreamReader(fs, Encoding.Default, true))
                    {
                        ...
                        var updatedContent = await UpdateSmartSoftwarePackagesAsync(fileContent,
                            includePreviews,
    ...
    public async Task UpdateProjectAsync(string projectPath, bool includeNightlyPreviews = false, bool includeReleaseCandidates = false, bool switchToStable = false, bool checkAll = false, string version = null)
    {
        ...
        {
            var latestVersionInfo = await _packageVersionCheckerService.GetLatestVersionOrNullAsync("SmartSoftware.Core");
            var latestReleaseCandidateVersionInfo = await _packageVersionCheckerService.GetLatestVersionOrNullAsync("SmartSoftware.Core", includeReleaseCandidates: true);
            var latestVersionFromMyGet = await GetLatestVersionFromMyGet("SmartSoftware.Core");
    ...
    protected virtual async Task UpdateInternalAsync(string projectPath, bool includeNightlyPreviews = false, bool includeReleaseCandidates = false, bool switchToStable = false)
    {
        using (var fs = File.Open(projectPath, FileMode.Open, FileAccess.ReadWrite, FileShare.None))
        {
            using (var sr = new StreamReader(fs, Encoding.Default, true))
            {
                ...
                var updatedContent = await UpdateSmartSoftwarePackagesAsync(fileContent, includeNightlyPreviews, includeReleaseCandidates, switchToStable);
    ...
    private async Task<string> UpdateSmartSoftwarePackagesAsync(string content,
        ...
        string specifiedVersion = null)
    {
        ...
        try
        {
            ...
            var packageNodeList = doc.SelectNodes("/Project/ItemGroup/PackageReference[starts-with(@Include, 'SmartSoftware.')]");
            ...
            if (packageNodeList != null)
            {
                foreach (XmlNode package in packageNodeList)
                {
                    ...
                    var isStudioPackage = packageId.StartsWith("SmartSoftware.Studio.");
                    if(isLeptonXPackage)
                    {
                    ...
                    else
                    {
                        if ((includeNightlyPreviews || (currentVersion.Contains("-preview") && !switchToStable)) && !includeReleaseCandidates)
                        {
                            ...
                            if(isLeptonXPackage)
                            {
                                ...
                                {
                                    //use LeptonX Lite package as the package name to be able to get the package version from the 'ss-nightly' feed.
                                    leptonXPackageName = "SmartSoftware.AspNetCore.Mvc.UI.Theme.LeptonXLite";
                                }
        ...
        {
            Logger.LogError("Cannot update SmartSoftware.* packages! An error occurred while updating the package \"{PackageId}\". Error: {ErrorMessage}", packageId, ex.Message);
