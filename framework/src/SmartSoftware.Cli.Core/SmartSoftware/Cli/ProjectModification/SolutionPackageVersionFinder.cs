...
{
    public string FindByDllVersion(string solutionFile, string dllName = "SmartSoftware*")
    {
    ...
    public string FindByCsprojVersion(string solutionFile, string packagePrefix = "SmartSoftware", string excludedKeywords = "LeptonX", string includedKeyword = null)
    {
        var projectFilesUnderSrc = GetProjectFilesOfSolution(solutionFile);
        foreach (var projectFile in projectFilesUnderSrc)
        {
    ...
    public static bool TryParseVersionFromCsprojFile(string csprojContent, out string version, string packagePrefix = "SmartSoftware", string excludedKeywords = "LeptonX", string includedKeywords = null)
    {
        return TryParseVersionFromCsprojViaXmlDocument(csprojContent, out version, packagePrefix, excludedKeywords, includedKeywords);
    }


    public static bool TryParseSemanticVersionFromCsprojFile(string csprojContent, out SemanticVersion version, string packagePrefix = "SmartSoftware", string excludedKeywords = "LeptonX", string includedKeywords = null)
    {
        try
        {
