...
public class ProjectNugetPackageAdder : ITransientDependency
{
        ...
        bool addSourceCodeToSolutionFile = false)
    {
        ...
        if (version == null)
        {
            version = GetSmartSoftwareVersionOrNull(projectFile);
        }
        ...
        bool useDotnetCliToInstall)
    {
        ...
        using (DirectoryHelper.ChangeCurrentDirectory(Path.GetDirectoryName(projectFile)))
        {
            ...
            var moduleFiles = ModuleClassFinder.Find(projectFile, "SmartSoftwareModule");
            if (moduleFiles.Count == 0)
            {
                throw new CliUsageException(
                    $"Could not find a class derived from SmartSoftwareModule in the project {projectFile}");
            }

            if (moduleFiles.Count > 1)
            {
                throw new CliUsageException(
                    $"There are multiple classes derived from SmartSoftwareModule in the project {projectFile}: " +
    ...
    protected virtual string GetSmartSoftwareVersionOrNull(string projectFile)
    {
        var projectFileContent = File.ReadAllText(projectFile);
    ...
    {
        var url = $"{CliUrls.WwwSmartSoftwareIo}api/app/nugetPackage/byName/?name=" + packageName;
