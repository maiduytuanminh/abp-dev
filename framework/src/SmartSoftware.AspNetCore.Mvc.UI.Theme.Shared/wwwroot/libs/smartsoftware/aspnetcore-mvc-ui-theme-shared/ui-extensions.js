...
(function () {
    ss.ui = ss.ui || {};
    ss.ui.extensions = ss.ui.extensions || {};

    ss.ui.extensions.ActionList = function () {
        return new ss.utils.common.LinkedList();
    };

    ss.ui.extensions.ColumnList = function () {
        return new ss.utils.common.LinkedList();
    };

    ss.ui.extensions.entityActions = (function () {
        var _callbackLists = {};
        ...
        function _get(name) {
            ...
            function _getActions() {
                var actionList = new ss.ui.extensions.ActionList();
    ...
    ss.ui.extensions.tableColumns = (function () {
        var _callbackLists = {};
        ...
        function _get(name) {
            ...
            function _getColumns() {
                var columnList = new ss.ui.extensions.ColumnList();
    ...
    function initializeObjectExtensions() {
        ...
        function localizeDisplayName(propertyName, displayName) {
            if (displayName && displayName.name) {
                return ss.localization.localize(displayName.name, displayName.resource);
            }

            if (ss.localization.isLocalized('DisplayName:' + propertyName)) {
                return ss.localization.localize('DisplayName:' + propertyName);
            }

            return ss.localization.localize(propertyName);
        }
        ...
        function localizeWithFallback(localizationResources, keys, defaultValue) {
            for (var i = 0; i < localizationResources.length; i++) {
                ...
                for (var j = 0; j < keys.length; j++) {
                    ...
                    if (ss.localization.isLocalized(key, localizationResource)) {
                        return ss.localization.localize(key, localizationResource);
                    }
                }
            }
        ...
        function localizeEnumMember(property, enumMemberValue) {
            ...
            var enumInfo = ss.objectExtensions.enums[enumType];
            ...
            return localizeWithFallback(
                [enumInfo.localizationResource, ss.localization.defaultResourceName],
                [
        ...
        function configureTableColumns(tableName, columnConfigs) {
            ss.ui.extensions.tableColumns.get(tableName)
                .addContributor(
                    function (columnList) {
        ...
        function convertPropertyToColumnConfig(property) {
            ...
            if (property.config.typeSimple === 'enum') {
                ...
                var defaultRenderer = ss.libs.datatables.defaultRenderers[property.config.typeSimple];
                if (defaultRenderer) {
                    columnConfig.render = function (data, type, row) {
                        var value = getValueFromRow(property, row);
        ...
        function configureEntity(moduleName, entityName, entityConfig) {
            ...
            if (tableProperties.length > 0) {
                var tableName = ss.utils.toCamelCase(moduleName) + "." + ss.utils.toCamelCase(entityName);
        ...
        var moduleNames = Object.keys(ss.objectExtensions.modules);

        for (var i = 0; i < moduleNames.length; i++) {
            configureModule(
                moduleNames[i],
                ss.objectExtensions.modules[moduleNames[i]]
            );
        }
    }
