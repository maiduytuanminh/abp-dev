...
(function ($) {
    ...
    var datatables = ss.utils.createNamespace(ss, 'libs.datatables');

    var localize = function (key) {
        return ss.localization.getResource('SmartSoftwareUi')(key);
    };
    ...
    (function () {
        ...
        var getVisibilityValue = function (visibilityField, record, tableInstance) {
            ...
            if (ss.utils.isFunction(visibilityField)) {
        ...
        var _createDropdownItem = function (record, fieldItem, tableInstance) {
            if (fieldItem.divider) {
                if (ss.utils.isFunction(fieldItem.divider)) {
            ...
            if (fieldItem.displayNameHtml) {
                $a.html(ss.utils.isFunction(fieldItem.text) ? fieldItem.text(record, tableInstance) : fieldItem.text);
            } else {
                ...
                $a.append(htmlEncode(ss.utils.isFunction(fieldItem.text) ? fieldItem.text(record, tableInstance) : fieldItem.text));
            }
                ...
                $a.click(function (e) {
                    ...
                    if (!$(this).closest('li').hasClass('disabled')) {
                        if (fieldItem.confirmMessage) {
                            ss.message.confirm(fieldItem.confirmMessage({ record: record, table: tableInstance }))
        ...
        var _createButtonDropdown = function (record, field, tableInstance) {
            if (field.items.length === 1) {
                ...
                var $button = $('<button type="button" class="btn btn-primary ss-action-button"></button>');

                if (firstItem.displayNameHtml) {
                    $button.html(ss.utils.isFunction(firstItem.text) ? firstItem.text(record, tableInstance) : firstItem.text);
                } else {
                    ...
                    $button.append(htmlEncode(ss.utils.isFunction(firstItem.text) ? firstItem.text(record, tableInstance) : firstItem.text));
                }
                    ...
                    $button.click(function (e) {
                        ...
                        if (!$(this).hasClass('disabled')) {
                            if (firstItem.confirmMessage) {
                                ss.message.confirm(firstItem.confirmMessage({ record: record, table: tableInstance }))
                                    .done(function (accepted) {
                                        if (accepted) {
            ...
            if (field.text) {
                $dropdownButton.append(htmlEncode(ss.utils.isFunction(field.text) ? field.text(record, tableInstance) : field.text));
            } else {
        ...
        var renderRowActions = function (tableInstance, nRow, aData, iDisplayIndex, iDisplayIndexFull) {
            ...
            } else if (ss.utils.isFunction(tableInstance.fnSettings)) {
                columns = tableInstance.fnSettings().aoColumns;
            }

            if (!columns && ss.utils.isFunction(tableInstance.api)) {
                    ...
                    if ($actionContainer) {
                        ...
                        for (var i = 0; i < cells.length; i++) {
                            ...
                            if (cell._DT_CellIndex && cell._DT_CellIndex.column === colIndex) {
                                var $actionButton = $(cell).find(".ss-action-button");
