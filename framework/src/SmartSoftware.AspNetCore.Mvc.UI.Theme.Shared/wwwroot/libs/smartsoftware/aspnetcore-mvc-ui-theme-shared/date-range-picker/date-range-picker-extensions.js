(function ($) {
    ...
    ss.libs.bootstrapDateRangePicker = {
        ...
        createDatePicker: function (options) {
            ...
            var $datePicker = options.singleDatePicker ? $('<ss-date-picker></ss-date-picker>') : $('<ss-date-range-picker></ss-date-range-picker>');
            ...
            ss.dom.initializers.initializeDateRangePickers($datePicker);
    ...
    function isEmptyDate(date, options) {
        ...
        return !SmartSoftwareDate(date, options).isValid();
    }

    function SmartSoftwareDate(originalDate, options, format) {

        var _moment = convertToMoment(originalDate, options, format);
        function _clone() {
            return SmartSoftwareDate(_moment, options, format);
        }

        function _isSame(date) {
            return _format(options.inputDateFormat) === SmartSoftwareDate(date, options).format(options.inputDateFormat);
        }
    ...
    function setOptions(options, $datePickerElement, singleDatePicker) {
        ...
        var defaultOptions = {
            ...
            locale: {
                direction: ss.localization.currentCulture.isRightToLeft ? 'rtl' : 'ltr',
                todayLabel: ss.localization.localize('Today', 'SmartSoftwareUi'),
                clearLabel: ss.localization.localize('Clear', 'SmartSoftwareUi'),
                applyLabel: ss.localization.localize('Apply', 'SmartSoftwareUi'),
            },
        ...
        if (options.ranges) {
            $.each(options.ranges, function (key, value) {
                ...
                options.ranges[key] = [SmartSoftwareDate(start, options), SmartSoftwareDate(end, options)];
            });
        }
    ...
    function createInitialSmartSoftwareDate(date, options) {
        date = convertToMoment(date, options, undefined, options.isUtc);
        if (options.isUtc) {
            date = date.local();
        }

        return SmartSoftwareDate(date, options);
    }

    ss.dom.initializers.initializeDateRangePickers = function ($rootElement) {
        $rootElement
            .findWithSelf('ss-date-picker,ss-date-range-picker')
            .each(function () {
                var $this = $(this);
                ...
                var singleDatePicker = $this.is('ss-date-picker')
                var options = {};
                ...
                var startDate = createInitialSmartSoftwareDate(options.startDate || options.date || (options.autoUpdateInput ? new Date() : undefined), options);

                var oldStartDate = SmartSoftwareDate(undefined, options);

                var endDate = createInitialSmartSoftwareDate(options.endDate || (options.autoUpdateInput ? new Date() : undefined), options);
                var oldEndDate = SmartSoftwareDate(undefined, options);
                ...
                function setDataDates(date, $selfInput, prefix){
                    date = SmartSoftwareDate(date, options).date;
                ...
                function setStartDateByMomentDate (momentDate) {
                    ...
                    startDate = SmartSoftwareDate(momentDate, options);
                }
                ...
                function setEndDateByMomentEndDate(momentDate){
                    ...
                    oldEndDate = endDate;
                    endDate = SmartSoftwareDate(momentDate, options);
                }
    ...
    ss.dom.onNodeAdded(function (args) {
        ss.dom.initializers.initializeDateRangePickers(args.$el);
    });

    $(function () {
        ss.dom.initializers.initializeDateRangePickers($('body'));
    });
})(jQuery);