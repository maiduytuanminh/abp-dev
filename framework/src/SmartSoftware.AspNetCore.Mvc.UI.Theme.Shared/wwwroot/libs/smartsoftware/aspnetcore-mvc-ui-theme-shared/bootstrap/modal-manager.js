...
(function ($) {
    ...
    ss.ModalManager = (function () {
        ...
        return function (options) {
            ...
            var _modalId = 'SmartSoftware_Modal_' + (Math.floor((Math.random() * 1000000))) + new Date().getTime();
            ...
            function _createContainer() {
                ...
                var existsModals = $('[id^="SmartSoftware_Modal_"]');
            ...
            function _initAndShowModal() {
                ...
                var modalClass = ss.modals[options.modalClass];
                if (modalClass) {
                ...
                _createContainer(_modalId)
                    .load(options.viewUrl, $.param(argsWithoutFunc), function (response, status, xhr) {
                        if (status === "error") {
                            ...
                            ss.ajax.showError(responseJSON.error ? responseJSON.error : ss.ajax.defaultError);
                            return;
                        };

                        if (options.scriptUrl) {
                            ss.ResourceLoader.loadScript(options.scriptUrl, function () {
                                _initAndShowModal();
                            });
                        } else {
                            _initAndShowModal();
