...
public class BundleContributorCollection
{
    ...
    private void AddInstanceToContributors(Type contributorType)
    {
        ...
        {
            throw new SmartSoftwareException($"Given {nameof(contributorType)} ({contributorType.AssemblyQualifiedName}) should implement the {typeof(IBundleContributor).AssemblyQualifiedName} interface!");
        }
        ...
        {
            throw new SmartSoftwareException($"Can not instantiate {contributorType.AssemblyQualifiedName}", ex);
        }
    }
}
