...
public class BundleConfigurationCollection
{
    ...
    /// Throws <see cref="SmartSoftwareException"/> if there is already a bundle with the same name.
    /// </summary>
        ...
        Action<BundleConfiguration>? configureAction = null)
    {
        ...
        {
            throw new SmartSoftwareException($"There is already a bundle added with given {nameof(bundleName)}: {bundleName}");
        }
    ...
    public BundleConfiguration Get([NotNull] string bundleName)
    {
        ...
        if (!_bundles.TryGetValue(bundleName, out var bundle))
        {
            throw new SmartSoftwareException($"There is no bundle with given {nameof(bundleName)}: {bundleName}");
        }
