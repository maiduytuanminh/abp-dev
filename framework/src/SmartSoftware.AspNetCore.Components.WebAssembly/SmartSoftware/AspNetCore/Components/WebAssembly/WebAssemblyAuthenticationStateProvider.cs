    ...
    protected virtual async Task TryRevokeOldAccessTokensAsync()
    {
        ...
        foreach (var accessToken in revokeAccessTokens)
        {
            ...
            if (!accessToken.IsNullOrWhiteSpace() && !currentAccessToken.IsNullOrWhiteSpace())
            {
                ...
                var currentSessionId = handler.ReadJwtToken(currentAccessToken)?.Claims?.FirstOrDefault(x => x.Type == SmartSoftwareClaimTypes.SessionId);
                var sessionId = handler.ReadJwtToken(accessToken)?.Claims?.FirstOrDefault(x => x.Type == SmartSoftwareClaimTypes.SessionId);
