...
public class SmartSoftwareAccordionTagHelperService : SmartSoftwareTagHelperService<SmartSoftwareAccordionTagHelper>
{
    protected IHtmlGenerator HtmlGenerator { get; }

    public SmartSoftwareAccordionTagHelperService(IHtmlGenerator htmlGenerator)
    {
    ...
    protected virtual void SetContent(TagHelperContext context, TagHelperOutput output, List<string> items)
    {
        ...
        {
            var content = item.Replace(SmartSoftwareAccordionParentIdPlaceholder, HtmlGenerator.Encode(TagHelper.Id));
