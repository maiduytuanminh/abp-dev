...
public abstract class SmartSoftwareDatePickerBaseTagHelperService<TTagHelper> : SmartSoftwareTagHelperService<TTagHelper>
    where TTagHelper : SmartSoftwareDatePickerBaseTagHelper<TTagHelper>
{
    ...
    protected readonly ISmartSoftwareTagHelperLocalizer TagHelperLocalizer;
    protected virtual string TagName { get; set; } = "ss-date-picker";
    protected IStringLocalizer<SmartSoftwareUiResource> L { get; }
    protected abstract TagHelperOutput TagHelperOutput { get; set; }

    protected SmartSoftwareDatePickerBaseTagHelperService(IJsonSerializer jsonSerializer, IHtmlGenerator generator,
        HtmlEncoder encoder, IServiceProvider serviceProvider, IStringLocalizer<SmartSoftwareUiResource> l,
        ISmartSoftwareTagHelperLocalizer tagHelperLocalizer)
    {
    ...
    public async override Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        var ssDatePickerTag = new TagHelperOutput(TagName, GetBaseTagAttributes(context, output, TagHelper),
            (_, _) => Task.FromResult<TagHelperContent>(new DefaultTagHelperContent()));
        ssDatePickerTag.Content.AppendHtml(inputGroup.Render(Encoder));
        ssDatePickerTag.Content.AppendHtml(validationContent);
        ssDatePickerTag.Content.AppendHtml(GetExtraInputHtml(context, output));

        var innerHtml = labelContent + ssDatePickerTag.Render(Encoder) + infoContent;
    ...
    protected TagHelperAttributeList ConvertDatePickerOptionsToAttributeList(ISmartSoftwareDatePickerOptions? options)
    {
        var attrList = new TagHelperAttributeList();
        ...
        {
            case SmartSoftwareDatePickerWeekNumbers.Normal:
                attrList.Add("data-show-week-numbers", "true");
                break;
            case SmartSoftwareDatePickerWeekNumbers.Iso:
        ...
        if (options.Opens != SmartSoftwareDatePickerOpens.Center)
        {
            attrList.Add("data-opens", options.Opens.ToString().ToLowerInvariant());
        }

        if (options.Drops != SmartSoftwareDatePickerDrops.Down)
        {
    ...
    protected TagHelperAttributeList GetBaseTagAttributes(TagHelperContext context, TagHelperOutput output, ISmartSoftwareDatePickerOptions options)
    {
        var groupPrefix = "group-";
    ...
    {
        var ssButtonTagHelper = ServiceProvider.GetRequiredService<SmartSoftwareButtonTagHelper>();
        var attributes =
            new TagHelperAttributeList { new("type", "button"), new("tabindex", "-1"), new("data-type", type) };
        ssButtonTagHelper.ButtonType = SmartSoftwareButtonType.Outline_Secondary;
        ssButtonTagHelper.Icon = icon;

        ssButtonTagHelper.Disabled = TagHelper.IsDisabled || GetAttribute<DisabledInput>() != null;
        ...
        return await ssButtonTagHelper.RenderAsync(attributes, context, Encoder, "button", TagMode.StartTagAndEndTag);
    }
    ...
    protected virtual string GetSize(TagHelperContext context, TagHelperOutput output)
    {
        ...
        return TagHelper.Size switch {
            SmartSoftwareFormControlSize.Small => "form-control-sm",
            SmartSoftwareFormControlSize.Medium => "form-control-md",
            SmartSoftwareFormControlSize.Large => "form-control-lg",
            _ => ""
        };
    }
