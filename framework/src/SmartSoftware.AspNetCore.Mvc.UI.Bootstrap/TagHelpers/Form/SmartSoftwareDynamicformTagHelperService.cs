...
using SmartSoftware.AspNetCore.Mvc.UI.Bootstrap.TagHelpers.Form.DatePicker;

namespace SmartSoftware.AspNetCore.Mvc.UI.Bootstrap.TagHelpers.Form;

public class SmartSoftwareDynamicFormTagHelperService : SmartSoftwareTagHelperService<SmartSoftwareDynamicFormTagHelper>
{
    protected HtmlEncoder HtmlEncoder { get; }
    protected IHtmlGenerator HtmlGenerator { get; }
    protected IServiceProvider _serviceProvider { get; }
    protected IStringLocalizer<SmartSoftwareUiResource> Localizer { get; }
    protected List<ModelExpression> Models = new();

    public SmartSoftwareDynamicFormTagHelperService(
    ...
    protected virtual void SetContent(TagHelperContext context, TagHelperOutput output, List<FormGroupItem> items, TagHelperContent childContent)
    {
        ...
        if (content.Contains(SmartSoftwareFormContentPlaceHolder))
        {
            content = content.Replace(SmartSoftwareFormContentPlaceHolder, contentBuilder.ToString());
        }
        else
        {
    ...
    {
        var ssDateTagHelper = GetDateGroupTagHelper(context, output, model);

        await ssDateTagHelper.RenderAsync(new TagHelperAttributeList(), context, HtmlEncoder, "div", TagMode.StartTagAndEndTag);
    }

    protected virtual SmartSoftwareTagHelper GetDateGroupTagHelper(TagHelperContext context, TagHelperOutput output, ModelExpression model)
    {
        return IsDateRangeGroup(model.ModelExplorer)
            ? GetSmartSoftwareDateRangeInputTagHelper(context, output, model)
            : GetSmartSoftwareDateInputTagHelper(model);
    }

    protected virtual SmartSoftwareTagHelper GetSmartSoftwareDateInputTagHelper(ModelExpression model)
    {
        var ssDateInputTagHelper = _serviceProvider.GetRequiredService<SmartSoftwareDatePickerTagHelper>();
        ssDateInputTagHelper.AspFor = model;
        ssDateInputTagHelper.ViewContext = TagHelper.ViewContext;
        ssDateInputTagHelper.DisplayRequiredSymbol = TagHelper.RequiredSymbols ?? true;
        return ssDateInputTagHelper;
    }

    protected virtual SmartSoftwareTagHelper GetSmartSoftwareDateRangeInputTagHelper(TagHelperContext context, TagHelperOutput output, ModelExpression model)
    {
        var modelAttribute = model.ModelExplorer.GetAttribute<DateRangePickerAttribute>()!;

        var pickerId = modelAttribute.PickerId;

        var ssDateRangeInputTagHelper = _serviceProvider.GetRequiredService<SmartSoftwareDateRangePickerTagHelper>();
        ssDateRangeInputTagHelper.PickerId = pickerId;
        ssDateRangeInputTagHelper.ViewContext = TagHelper.ViewContext;

        if (modelAttribute.IsStart)
        {
            ssDateRangeInputTagHelper.AspForStart = model;
            ...
            {
                ssDateRangeInputTagHelper.AspForEnd = ModelExplorerToModelExpressionConverter(otherModel!);
            }
        }

        return ssDateRangeInputTagHelper;
    }
    ...
    {
        var ssSelectTagHelper = GetSelectGroupTagHelper(context, output, model);

        await ssSelectTagHelper.RenderAsync(new TagHelperAttributeList(), context, HtmlEncoder, "div", TagMode.StartTagAndEndTag);
    }

    protected virtual SmartSoftwareTagHelper GetSelectGroupTagHelper(TagHelperContext context, TagHelperOutput output, ModelExpression model)
    {
        return IsRadioGroup(model.ModelExplorer) ?
            GetSmartSoftwareRadioInputTagHelper(model) :
            GetSelectTagHelper(model);
    }

    protected virtual SmartSoftwareTagHelper GetSelectTagHelper(ModelExpression model)
    {
        var ssSelectTagHelper = _serviceProvider.GetRequiredService<SmartSoftwareSelectTagHelper>();
        ssSelectTagHelper.AspFor = model;
        ssSelectTagHelper.AspItems = null;
        ssSelectTagHelper.ViewContext = TagHelper.ViewContext;
        return ssSelectTagHelper;
    }

    protected virtual SmartSoftwareTagHelper GetSmartSoftwareRadioInputTagHelper(ModelExpression model)
    {
        var radioButtonAttribute = model.ModelExplorer.GetAttribute<SmartSoftwareRadioButton>()!;
        var ssRadioInputTagHelper = _serviceProvider.GetRequiredService<SmartSoftwareRadioInputTagHelper>();
        ssRadioInputTagHelper.AspFor = model;
        ssRadioInputTagHelper.AspItems = null;
        ssRadioInputTagHelper.Inline = radioButtonAttribute.Inline;
        ssRadioInputTagHelper.Disabled = radioButtonAttribute.Disabled;
        ssRadioInputTagHelper.ViewContext = TagHelper.ViewContext;
        return ssRadioInputTagHelper;
    }
    ...
    {
        var ssButtonTagHelper = _serviceProvider.GetRequiredService<SmartSoftwareButtonTagHelper>();
        var attributes = new TagHelperAttributeList { new TagHelperAttribute("type", "submit") };
        ssButtonTagHelper.Text = Localizer["Submit"];
        ssButtonTagHelper.ButtonType = SmartSoftwareButtonType.Primary;

        return await ssButtonTagHelper.RenderAsync(attributes, context, HtmlEncoder, "button", TagMode.StartTagAndEndTag);
    }
    ...
    {
        var ssInputTagHelper = _serviceProvider.GetRequiredService<SmartSoftwareInputTagHelper>();
        ssInputTagHelper.AspFor = model;
        ssInputTagHelper.ViewContext = TagHelper.ViewContext;
        ssInputTagHelper.DisplayRequiredSymbol = TagHelper.RequiredSymbols ?? true;

        await ssInputTagHelper.RenderAsync(new TagHelperAttributeList(), context, HtmlEncoder, "div", TagMode.StartTagAndEndTag);
    }
    ...
    {
        return explorer.GetAttribute<SmartSoftwareRadioButton>() != null;
    }
}
