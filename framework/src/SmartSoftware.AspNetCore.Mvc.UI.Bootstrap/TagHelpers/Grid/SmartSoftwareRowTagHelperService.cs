...
public class SmartSoftwareRowTagHelperService : SmartSoftwareTagHelperService<SmartSoftwareRowTagHelper>
{
    public override void Process(TagHelperContext context, TagHelperOutput output)
    {
        if (output.TagName == "ss-row")
        {
            output.Attributes.AddClass("row");
        }
        if (output.TagName == "ss-form-row")
        {
            output.Attributes.AddClass("row");
        }
