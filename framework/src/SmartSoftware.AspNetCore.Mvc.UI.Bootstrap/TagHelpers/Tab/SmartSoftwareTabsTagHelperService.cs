...
public class SmartSoftwareTabsTagHelperService : SmartSoftwareTagHelperService<SmartSoftwareTabsTagHelper>
{
    protected IHtmlGenerator HtmlGenerator { get; }

    public SmartSoftwareTabsTagHelperService(IHtmlGenerator htmlGenerator)
    {
    ...
    protected virtual void SetActiveTab(List<TabItem> items)
    {
        ...
        foreach (var tabItem in items)
        {
            if (tabItem.Active)
            {
                tabItem.Content = tabItem.Content.Replace(SmartSoftwareTabItemShowActivePlaceholder, " show active");
                tabItem.Header = tabItem.Header.Replace(SmartSoftwareTabItemActivePlaceholder, " active").Replace(SmartSoftwareTabItemSelectedPlaceholder, "true");
            }
            else
            {
                tabItem.Content = tabItem.Content.Replace(SmartSoftwareTabItemShowActivePlaceholder, "");
                tabItem.Header = tabItem.Header.Replace(SmartSoftwareTabItemActivePlaceholder, "").Replace(SmartSoftwareTabItemSelectedPlaceholder, "false");
            }
        }
    ...
    protected virtual string GetHeaders(TagHelperContext context, TagHelperOutput output, List<TabItem> items)
    {
        ...
        for (var index = 0; index < items.Count; index++)
        {
            ...
            if (item.IsDropdown)
            {
                ...
                header = item.Header.Replace(SmartSoftwareTabDropdownItemsActivePlaceholder, childHeadersAsString);
            }
