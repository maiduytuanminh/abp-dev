...
public class SmartSoftwareDropdownButtonTagHelperService : SmartSoftwareTagHelperService<SmartSoftwareDropdownButtonTagHelper>
{

    private readonly HtmlEncoder _htmlEncoder;
    private readonly IServiceProvider _serviceProvider;

    public SmartSoftwareDropdownButtonTagHelperService(
    ...
    protected virtual async Task<string> GetMainButtonAsync(TagHelperContext context, TagHelperOutput output, TagHelperContent content)
    {
        ...
        ssButtonTagHelper.Icon = TagHelper.Icon;
        ssButtonTagHelper.Text = TagHelper.Text;
        ssButtonTagHelper.IconType = TagHelper.IconType;
        ssButtonTagHelper.Size = TagHelper.Size;
        ssButtonTagHelper.ButtonType = TagHelper.ButtonType;
        var attributes = GetAttributesForMainButton(context, output);

        var buttonTag = await ssButtonTagHelper.ProcessAndGetOutputAsync(attributes, context, "button", TagMode.StartTagAndEndTag);
    ...
    protected virtual async Task<string> GetSplitButtonAsync(TagHelperContext context, TagHelperOutput output)
    {
        ...
        return await ssButtonTagHelper.RenderAsync(attributes, context, _htmlEncoder, "button", TagMode.StartTagAndEndTag);
    }
