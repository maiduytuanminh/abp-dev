...
public abstract class DistributedEventBusBase : EventBusBase, IDistributedEventBus, ISupportsEventBoxes
{
        ...
        eventHandlerInvoker)
    {
        ...
        Clock = clock;
        SmartSoftwareDistributedEventBusOptions = ssDistributedEventBusOptions.Value;
    ...
    protected virtual async Task<bool> AddToOutboxAsync(Type eventType, object eventData)
    {
        ...
        foreach (var outboxConfig in SmartSoftwareDistributedEventBusOptions.Outboxes.Values.OrderBy(x => x.Selector is null))
        {
    ...
    {
        if (SmartSoftwareDistributedEventBusOptions.Inboxes.Count <= 0)
        {
            return false;
        }
        ...
        {
            foreach (var inboxConfig in SmartSoftwareDistributedEventBusOptions.Inboxes.Values.OrderBy(x => x.EventSelector is null))
            {
