...
public class HasExtraPropertiesObjectExtendingExtensions_Tests : SmartSoftwareObjectExtendingTestBase
{
    ...
    public void MapExtraPropertiesTo_Should_Validate_Properties()
    {
        using (CultureHelper.Use(CultureInfo.InvariantCulture))
        {
            ...
            Assert.Throws<SmartSoftwareValidationException>(() => person.MapExtraPropertiesTo(personDto)).ValidationErrors.ShouldContain(x => x.ErrorMessage == "The field Name must be a string with a maximum length of 4.");
        }
