...
public class GitRepositoryBuildStatus_Tests : SmartSoftwareCliTestBase
{
    ...
    {
        var existingBuildStatus = new GitRepositoryBuildStatus("smartsoftware", "dev")
        {
    ...
    public void Update_Existing_Build_Status_Test()
    {
        ...
        existingBuildStatus.MergeWith(newBuildStatus);
        existingBuildStatus.SucceedProjects.Count.ShouldBe(2);
        existingBuildStatus.GetSelfOrChild("smartsoftware").SucceedProjects.First(p => p.CsProjPath == "project1.csproj")
            .CommitId.ShouldBe("2");
        existingBuildStatus.GetSelfOrChild("smartsoftware").SucceedProjects.First(p => p.CsProjPath == "project2.csproj")
            .CommitId.ShouldBe("2");
    }
    ...
    public void Add_New_Build_Status_For_Child_Repository_Test()
    {
        var existingBuildStatus = new GitRepositoryBuildStatus("smartsoftware", "dev")
        {
            DependingRepositories = new List<GitRepositoryBuildStatus>()
                {
                    new GitRepositoryBuildStatus("ss", "dev")
                    {
                        SucceedProjects = new List<DotNetProjectBuildStatus>
                        {
                            new DotNetProjectBuildStatus
        ...
        existingBuildStatus.MergeWith(newBuildStatus);
        existingBuildStatus.GetChild("ss").SucceedProjects.Count.ShouldBe(2);
    }
