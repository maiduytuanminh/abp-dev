...
public class AspNetCoreMultiTenancy_Without_DomainResolver_Tests : AspNetCoreMultiTenancyTestBase
{
    ...
    {
        var result = await GetResponseAsObjectAsync<Dictionary<string, string>>("http://smartsoftware.io");
    ...
    public async Task Should_Use_QueryString_Tenant_Id_If_Specified()
    {
        ...
        var result = await GetResponseAsObjectAsync<Dictionary<string, string>>($"http://smartsoftware.io?{_options.TenantKey}={_testTenantName}");
