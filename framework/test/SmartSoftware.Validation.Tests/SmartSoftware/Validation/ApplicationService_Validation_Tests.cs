...
public class ApplicationService_Validation_Tests : SmartSoftwareIntegratedTest<ApplicationService_Validation_Tests.TestModule>
{
    ...
    {
        await Assert.ThrowsAsync<SmartSoftwareValidationException>(async () => await _myAppService.MyMethod(new MyMethodInput())); //MyStringValue is not supplied!
        await Assert.ThrowsAsync<SmartSoftwareValidationException>(async () => await _myAppService.MyMethod(new MyMethodInput { MyStringValue = "a" })); //MyStringValue's min length should be 3!
    }
    ...
    {
        await Assert.ThrowsAsync<SmartSoftwareValidationException>(async () =>
    ...
    //TODO: Create a SmartSoftware.Ddd.Application.Contracts.Tests project and move this to there and remove SmartSoftware.Ddd.Application.Contracts dependency from this project.
    [Fact]
    public async Task LimitedResultRequestDto_Should_Throw_Exception_For_Requests_More_Than_MaxMaxResultCount()
    {
        var exception = await Assert.ThrowsAsync<SmartSoftwareValidationException>(async () =>
        {
